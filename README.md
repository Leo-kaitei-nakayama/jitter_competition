...
| density-weighted resampling of a candidate union (`resample_uniform.py --method weighted`) | −2.69: independent draws cannot enforce spacing, so neighbouring candidates win together (output p5 spacing collapsed 4×) and sparse-preference favours off-surface stragglers (P2S −1.5). |
| blue-noise resampling of the same union (`--method fps`) | the selector worked (p5 spacing doubled instead of collapsing) and still lost: −0.78 vs the raw input set, −1.78 vs repulsion, high bins flat. The σ-variant union carries no real diversity — the same ensemble correlation that made averaging worthless makes its holes shared, and selection cannot place a point where no candidate exists. **Selection loses to motion**: the repulsion filter moves mass into holes; a selector can only shuffle who stands where. Any revival needs genuinely diverse candidates (jitter-TTA re-denoise), which costs 2–3× inference against round B's efficiency grade — deprioritised. |
